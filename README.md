...
the key is to have all the style objects **predefined**, as opposed to creating a new object each time the meta expression is evaluated, which would be inefficient. in theory the providence of the style objects could be anywhere (`Form`, process variables, `Storage`...) but since the styles are an intrinsic part of the listbox object, it makes sense to define them as propeties of a class that is associated with this listbox. 
